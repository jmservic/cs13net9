partial class Program
{
    private static void DeferredExecution(string[] names)
    {
        ...
        WriteLine("Enumerating over the query");
        foreach (string name in query1)
        {
            WriteLine(name); // outputs Pam
            names[2] = "Jimmy"; // Change Jim to Jimmy.
            // On sthe second iteration Jimmy does not end with an "m" so it does not get output.
        }
    }   
    ...
    private static void FilteringUsingWhere(string[] names)
    {
        ...
        IOrderedEnumerable<string> query2 = from name in names where name.Length > 4 orderby name.Length, name select name;
    ...
    static void WorkingWithSets()
    {
        ...
        string[] cohort3 = { "Declain", "Jack", "Jack", "Jasmine", "Conor" };
