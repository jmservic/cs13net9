...
#region "Generating random numbers with the Random class
...
#region "New random methods in .NET 8 and later"
string[] beatles = r.GetItems(choices: new[] { "John", "Paul", "George", "Ringo" },
length: 10);
...
int num = 0x9ABCEF;
foreach (byte b in BitConverter.GetBytes(num)) { // Memory is little endian. That's why the GUID byte array is backwards for the first 3 sections.
    Write($"{b:X2} ");
}
#endregion